...
export default function Home() {
    ...
    const handleScroll = (e) => {
        ...
        if (!isScroll.current) {
            ...
            window.requestAnimationFrame(() => {
                setScrollPosition(Math.min(y, scrollMax));
    ...
    return (
        <div className="flex justify-center items-start min-h-screen bg-[#dae1e6]">
            <div
                id="box"
                onScroll={handleScroll}
                className="relative bg-[#00010E] rounded-4xl overflow-y-auto overflow-x-hidden w-4/5 min-h-[80vh]"
            >
                <div
                    className="stars absolute top-0 left-0 w-full h-full z-0 pointer-events-none"
                    style={{ transform: `translateY(${scrollPosition}px)` }}
                >
                    <Star />
                </div>
                <div
                    className="flex flex-col items-center absolute top-0 w-full text-xl text-[#ff7699] p-4 text-center"
                    style={{ transform: `translateY(${translateY * 0.1}px)`, transition: "transform 0.3s" }}
                >
                    <TypeAnimation
                        sequence={[
